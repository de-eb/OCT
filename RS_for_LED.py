...
for i in range(len(wl)):
    ref=spw[i]*np.sin(one_cycle)

    #light from surface
    light_sur=spw[i]*R*np.sin(one_cycle+phase_diff.values[i])

    #light throught the 1st cellophane
    lp1=(((2*tc)%wl_c.values[i])/wl_c.values[i])*2*np.pi
    light1=spw[i]*R*T**2*np.sin(one_cycle+phase_diff.values[i]+lp1)

    #light throught the 2nd cellophane
    light2=spw[i]*R*T**4*np.sin(one_cycle+phase_diff.values[i]+lp1*2)
    #light2=0
...
itf_rmbg=itf-sp
plt.plot(wl,itf_rmbg)
plt.show()
freq_fixed,itf_fixed=Resampling(wl,itf_rmbg)
