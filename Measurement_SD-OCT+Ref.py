...
if __name__ == "__main__":
    ...
    while g_key != 'escape':
        ...
        """ ステージの操作コマンド一覧 """
        if g_key in ['4','6','5','2','8']:                  # 自動ステージ（試料ホルダー）の位置調整
        ...
        if g_key in ['7','9','1','3','0']:                  # ピエゾステージ（参照ミラー）の位置調整
        ...
        if g_key == '/':                                    # '/' キーでステージを左端に移動する
            stage_s.absolute_move(-71000)
        

        """ リアルタイム測定 """
        try: itf[0,:] = ccs.read_spectra(averaging = 5)     # <CCS測定>（光強度が飽和しているとエラーメッセージ）
            ...
            if ccs_err:
                ...
                ccs_err = False
        ax0_0.set_data(ccs.wavelength[ccs_st:ccs_ed], itf[0, ccs_st:ccs_ed])
        ...
        try: reflect[0,:] = pma.read_spectra(averaging = 5) # <PMA測定>（光強度が飽和しているとエラーメッセージ）
        ...
        if incident is not None:                            # 信号処理（リアルタイムでの反射率）
            reflectance = calculate_reflectance(reflect[0, pma_st:pma_ed], incident[pma_st:pma_ed])
        ...
        if g_key == 'g':                                    # 'g' キーでリファレンスとa-scanのデータを削除する  
            reference = None
        ...
        if g_key == 'x':                                    # 'x' キーでリファレンスのデータを保存する
            ...
            if incident is None:
                ...
                dh.save_spectra(wavelength = pma.wavelength, reference = incident, spectra = data, memo = 'Attention:This is reflectance measurement data.')
        ...
        if g_key =='c' and stage_s_flag:                    # 'c' キーで測定開始（2次元のデータ）
            if incident is None or stage_s_flag == False:
                ...
                result_map = calculate_reflectance_2d(reflect[:,pma_st:pma_ed], incident[pma_st:pma_ed])
            ...
            if incident is None or stage_s_flag == False:
                ...
                for i in tqdm (range(step_v)):
                    for j in range(step_h):
                        ...
                        stage_s.relative_move(int(width/step_h*pl_rate*(-1)))
                    stage_s.biaxial_move(v = int(height/step_v*pl_rate*(-1)), vmode = 'r', h = int((width*pl_rate/2)), hmode = 'a')
                dh.save_spectra_3d(wavelength = pma.wavelength, width = width, height = height, reference = incident, spectra = reflect, memo = memo+'Attention:This is reflectance measurement data.')

        if g_key == 'b':                                    # 'b' キーでリファレンスのデータを削除する
            incident = None
        ...
        if g_key == 'e' and stage_s_flag:                   # 'e' キーで測定開始（2次元のデータ）
            if incident is None or reference is None or stage_s_flag == False:
                ...
                result_abs = calculate_reflectance_2d(reflection = reflect)
                result_oct = sp.generate_bscan(itf[:,ccs_st:ccs_ed], reference[ccs_st:ccs_ed])
                dh.save_spectra(wavelength = pma.wavelength, reference = incident, spectra = reflect.T, memo = 'Attention:This is reflectance measurement data.')
