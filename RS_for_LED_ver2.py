...
#Resampling wavelength characteristic
freq,sp_fixed=Resampling(wl,sp)
...
for i in range(len(freq)):
    #reference light
    ref=sp_fixed[i]*np.sin(one_cycle)
    
    #light from surface
    light_sur=sp_fixed[i]*R*np.sin(one_cycle+phase_diff[i])

    #light throught the 1st cellophane
    lp1=(((2*tc)%wl_c[i])/wl_c[i])*2*np.pi
    light1=sp_fixed[i]*R*T**2*np.sin(one_cycle+phase_diff[i]+lp1)

    #light throught the 2nd cellophane
    light2=sp_fixed[i]*R*T**4*np.sin(one_cycle+phase_diff[i]+lp1*2)
...
plt.show()
'''
depth,result=inverse_ft(freq,itf,0.2,n2)
